...
brands = [
    ...
    'Premiata will be', 'Principe Di Bologna', 'RBRSL', "Reptile's House", 'Roberto Cavalli', 'Rocco P', 'Sergio Rossi',
    'SPRAYGROUND', 'Stemar', 'Stuart Weitzman', 'V SEASON', "VIC MATIE'", "VIC MATIE", 'Voile Blanche', 'What For',
    'Wolford', '3JUIN', 'Sprayground'
]
...
def open_brand(el):
    driver.find_element_by_xpath('//div[@class="js-quick-search-source brand-popover"]'
...
def search():
    ...
    for b in search_values:
        ...
        driver.find_element_by_xpath('//input[@id="Search-q"]').click()
        driver.find_element_by_xpath('//input[@id="Search-q"]').clear()
        driver.find_element_by_xpath('//input[@id="Search-q"]').send_keys(b)
        driver.find_element_by_xpath('//input[@id="Search-q"]').send_keys(Keys.ENTER)
        time.sleep(2)
        try:
            write_data()
        except Exception as e:
...
def change_page():
    try:
        ...
        change_page = driver.find_element_by_xpath(show)
        actions = ActionChains(driver)
...
def get_data():
    ...
    for el in elems:
        ...
        try:
            ...
            try:
                WebDriverWait(driver, 10).until(EC.visibility_of_element_located)
                price_old = el.find_element_by_xpath('//ul[@class="list-items list-view-1 js-list-items"]'
...
def run():
    write_file(url_brands, 'rendez-vous_brands_full', params=1)
    write_file(url_brands, 'rendez-vous_brands_full', params=2)
